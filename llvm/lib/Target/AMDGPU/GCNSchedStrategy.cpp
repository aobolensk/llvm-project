...
bool GCNMaxOccupancySchedStrategy::tryCandidate(SchedCandidate &Cand,
  ...
  // In a region already known to exceed the pressure limits we sit on the
  // occupancy knee, where one register tier is worth more than any bubble or
  // clustering/weak-edge tie-break. Promote the whole-region max pressure
  // check above the stall, cluster and weak checks here only. KnownExcessRP is
  // fixed for the whole region, so the ordering stays consistent across every
  // comparison made while scheduling it.
  if (KnownExcessRP && DAG->isTrackingPressure() &&
  ...
  if (SameBoundary) {
    ...
    // heuristics to take precedence. Chasing ILP this hard is the wrong trade
    // in a region that is already over the pressure limits, so skip it there.
    if (Rem.IsAcyclicLatencyLimited && !KnownExcessRP && !Zone->getCurrMOps() &&
